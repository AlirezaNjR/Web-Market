  ...
  (function () {
    ...
    $container.imagesLoaded(function () {
      ...
      var updateIsotopeFiltering = function () {
        var selectedElms = $(".sidebar-filters .in")
            .find(".selectable.selected[data-target]")
            .not(".detailed"),
          detailedElms = $(".sidebar-filters .in").find(
            ".detailed.selected[data-target]"
          ),
          filterString,
          filter,
          types = [];
        ...
        if ($slider.parents(".accordion-body").hasClass("in")) {
          filter = filter.filter(function () {
            var $this = $(this);
            return (
              $this.data("price") >= $slider.slider("values", 0) &&
              $this.data("price") <= $slider.slider("values", 1)
            );
          });
        }
      ...
      $("#isotopeSorting").change(function () {
        ...
        parameters.sortAscending =
          "true" === parameters.sortAscending ? true : false;
  ...
  (function () {
    ...
    tour.addSteps([
      ...
      },
      {
        ...
        title: "دسته بندی ها",
        content:
          "روی دسته بندی ها کلیک کنید، محتویات به صورت اتوماتیک نمایان/مخفی میشوند.",
      },
      {
        element: "#filterPrices",
        title: "فیلتر بازه قیمت",
        content:
          "بازه قیمت را انتخاب کنید، خواهید دید که بدون ریفرش کردن صفحه، محصولات موجود در باره نمایش داده میشوند.",
      },
      {
        element: "#tourStep3",
        title: "باز کردن فیلتر های دیگر",
        content:
          "با یک کلیک روی عنوان، پنل را باز کنید و فیلتر کردن را شروع کنید. <br/> به همان شکل میتوانید فیلتر ها را حذف کنید، کافی است پنل را ببندید.",
        placement: "top",
      },
      {
        element: "#tourStep4",
        ...
        content:
          "برای مرتب سازی، اصلا نیازی به ریفرش کردن صفحه نیست. همه ی تغییرات به صورت آنی اعمال می شوند و شما میتوانید نتیجه را ببینید!",
        placement: "bottom",
      },
    ]);
    tour.start();
  ...
  $(".add-googlemap").each(function () {
    ...
    if (jQuery.goMap) {
      $this.goMap({
        markers: [
          {
            ...
            title:
              "undefined" === typeof $this.data("title")
                ? false
                : $this.data("title"),
          },
        ],
        scrollwheel: false,
        zoom:
          "undefined" === typeof $this.data("zoom")
            ? 13
            : parseInt($this.data("zoom"), 10),
        maptype:
          "undefined" === typeof $this.data("type")
            ? "ROADMAP"
            : $this.data("type").toUpperCase(),
      });
    }
  });
  ...
  var triggeredOnResize = function () {
    ...
    $('iframe[src*="vimeo.com"], iframe[src*="youtube.com"]').each(function () {
      ...
      $this.css(
        "height",
        parseInt(
          ($this.width() * $this.attr("height")) / $this.attr("width"),
          10
        )
      );
    });
    ...
    (function () {
      ...
      if ($(".large-screen #mainNavigation > .active").length > 0) {
        $magicLine
          .width($(".large-screen #mainNavigation > .active").width())
